...
def gen(camera):
    ...
    frame_size = (int(camera.video.get(3)), int(camera.video.get(4)))
    frame_midpoint = (int(frame_size[0]/2), int(frame_size[1]/2))
    map_keys(camera.map, camera.split)    

    with ExitStack() as stack:
        pose_models = camera.split*[stack.enter_context(mp.solutions.pose.Pose())]

        while camera.video.isOpened():
            success, image = camera.video.read()
            if not success: break

            image = process_poses(image, pose_models, camera.draw_landmarks, camera.flip, camera.display)
            yield (b'--frame\r\n'
